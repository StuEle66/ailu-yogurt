...
flowchart LR
  ...
  A --> G["专用 Chrome"] --> H["公众号长文章草稿"]
  A -. 可选 .-> R["自建 wechat-relay"] --> H
...
打开 **Obsidian 设置 → Ailu → 草稿**，选择 **专用 Chrome（推荐）**。首次点击“上传到草稿箱”时，Ailu 会打开独立 Chrome Profile；在该窗口登录公众号后重新开始操作。

Ailu 会先执行本地预检，再把冻结的标题、正文、正文图片和封面填入公众号长文章编辑器。只有通过最终确认才会点击“保存为草稿”，绝不会点击群发或正式发布。若页面要求验证码、控件发生变化、图片数量不符或保存结果不明确，Ailu 会保留编辑页并停止；先人工核对草稿箱，不要盲目重试。

不需要服务器的用户到此即可。已经部署固定公网出口和微信 API 白名单的用户，可以继续配置下面的自托管中转。

### 1.1 可选：配置公众号服务器与中转服务
